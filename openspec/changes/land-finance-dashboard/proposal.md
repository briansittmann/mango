...
- Split the dashboard into the units §9 describes (month selector, free-margin hero, the three expandable summary cards, category card, account avatar, account menu, pie and month-over-month charts). A single category card component renders the "Gastos fijos" group and every spending category.
- Define a typed dashboard data contract (numbers and ISO dates, never formatted strings). Every component receives its data and action handlers through props, and none of them imports a data-access module. A lint rule enforces this.
...
- **Code:** new `components/dashboard/*`, `lib/datos/dashboard.ts` (contract), `lib/datos/presupuesto.ts` (pure budget maths), `lib/demo/datos-demo.ts`, `app/demo/page.tsx`, `i18n/request.ts`, `messages/{es,en}.json`, `lib/utils.ts`. Modified: `app/layout.tsx`, `app/globals.css`, `next.config.ts`, `eslint.config.mjs`, `package.json`. Removed: `components/finance-dashboard.tsx`.
