...
export function AddRow({ label, onClick }: AddRowProps) {
  return (
    ...
    >
      <span className="grid size-7 shrink-0 place-items-center rounded-full bg-brand/15 text-brand-ink transition-[background-color_150ms_ease-out,color_150ms_ease-out] group-hover/add:bg-brand group-hover/add:text-primary-foreground motion-safe:transition-[background-color_150ms_ease-out,color_150ms_ease-out,scale_400ms_var(--ease-spring)] motion-safe:group-hover/add:scale-110 motion-safe:group-active/add:scale-90">
        <Plus
          aria-hidden
          strokeWidth={2.5}
