...
*Amendment (2026-09-24):* Supabase no longer lets the templates be edited without custom SMTP, so until SMTP is configured the e-mail carries the default `{{ .ConfirmationURL }}`. `sendMagicLink` passes `emailRedirectTo: <origin>/auth/confirm`, and `/auth/confirm` also accepts `?code=` and calls `exchangeCodeForSession`. The `token_hash` path is unchanged and takes over as soon as the template is edited. Until then the link works only in the browser that requested it.
...
*Amendment (2026-09-24):* RLS `with check` only constrains the written row's `usuario_id`, so a direct insert could reference another user's category. `0019_referencias_mismo_usuario.sql` makes every such reference a composite FK with `usuario_id` (`transacciones.categoria_id`, `transacciones.movimiento_recurrente_id`, `movimientos_recurrentes.categoria_id`, `presupuestos.categoria_id`), so the database rejects it for the web, the bot and direct API calls alike. Delete rules are unchanged.
