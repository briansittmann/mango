...
- Hecho: el margen libre usa sobres (`fix-free-margin-envelope-budgets`): `getFreeMargin` en `lib/data/budget.ts` descuenta fijos, `max(presupuesto, gastado)` por categoria con presupuesto y lo gastado sin presupuesto; la barra de presupuesto deja afuera los cobros recurrentes. Presupuestos por ciclo (`BudgetRow`, copia al ciclo actual) implementados solo en `lib/demo/demo-budgets.ts`; `/demo` arranca en 844 €. El boton de ciclo siguiente queda deshabilitado en el ciclo en curso.
