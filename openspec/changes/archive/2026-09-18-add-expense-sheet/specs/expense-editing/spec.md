...
- **AND** the "comida" card shows 247,60 € of 400 €, the expenses total shows 1.637,60 € and the free margin shows 1.036,40 €

#### Scenario: Restore brings the expense back
- **WHEN** the same expense is restored
- **THEN** "Café" is listed in "comida" with 62,40 € and its date, the card shows 310 € of 400 €, and the free margin shows 974 €
...
- **AND** a delete action is shown below the fields
...
- **AND** only edit mode shows a divider and the delete action below the fields
...
**Fixed-expenses card.** Its rows are single charges of the current cycle.
- Activating a fixed-expenses row SHALL open edit mode for that charge only. The header SHALL state that only this month's charge changes.
- Saving or deleting a fixed charge SHALL NOT change the recurring definition it came from, that definition's active state, or the charge of any other cycle.
- The fixed-expenses card SHALL NOT have an add row.

#### Scenario: Fixed charge edits this month only
- **WHEN** the visitor opens the fixed-expenses card on `/demo` in Spanish and taps "Alquiler" (820 €)
- **THEN** the sheet opens in edit mode with a header caption reading "Gastos fijos · Solo el cargo de este mes"
- **AND** after changing the amount to 880 and saving, the fixed-expenses card total is 960 € and the free margin is 914 €

#### Scenario: Fixed charge update carries only the charge
- **WHEN** a recording implementation of the operations is mounted and the user saves an edit to a fixed-expenses row
- **THEN** the only call is update, with that expense's identifier, amount, description and date

#### Scenario: No add row on the fixed card
- **WHEN** the fixed-expenses card is open
- **THEN** its last row is an expense row, and no "add expense" row is rendered
...
- **WHEN** on `/demo` in Spanish, with the free margin at 974 €, the visitor adds an expense of 20 described as "Panadería" to "comida"
- **THEN** the sheet closes and "Panadería" is listed in "comida" with 20 €
- **AND** the card shows 330 € of 400 € with the "near limit" label, the pie centre shows 1.720 €, and the free margin shows 954 €
...
- **AND** on release the row slides out and collapses, the "transporte" card shows 50 € of 100 €, and the undo toast appears
...
- **WHEN** the visitor opens "Farmacia" in "salud" and activates "Eliminar gasto"
