  ...
  return (
    <div className="rounded-card border border-border bg-card p-5">
      <div className="flex items-baseline justify-between">
        ...
        <Money amount={total} currency={currency} className="text-tabular-numeric-md text-foreground" />
      </div>
      <div className="mt-5 flex items-center gap-5">
        <div className="relative size-28 shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={slices} dataKey="total" nameKey="id" innerRadius="71%" outerRadius="100%" stroke="var(--card)" strokeWidth={2}>
                {slices.map((slice) => (
        ...
        <div className="flex flex-1 flex-col gap-2 text-body-md">
          {slices.map((slice) => (
            <div key={slice.id} className="flex items-center gap-2">
              ...
              <Money amount={slice.total} currency={currency} className="text-tabular-numeric-md" />
            </div>
          ))}
        </div>
      </div>
