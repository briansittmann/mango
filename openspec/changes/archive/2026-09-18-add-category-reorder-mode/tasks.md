...
- [x] 10.2 Run the drag on a real iPhone in Safari: measure the frame rate while dragging a card the length of the list with the overlay active, and confirm the settle does not feel dead in the hand without velocity handoff (D5, Risks). If either fails, record which fallback applies — dropping the blur, or a spring runtime for the settle — before proceeding.

  **Verified by the user on a real iPhone in Safari**: works well — no frame-rate drop dragging the length of the list with the overlay active, and the settle does not feel dead without velocity handoff. Neither fallback (dropping the blur, a spring runtime for the settle) is needed.

## 11. Docs and tests

- [x] 11.1 Add the **Reorder Mode** entry to `DESIGN.md` (handle anatomy, pushed-back layer recipe and its reduced-transparency variant, lift treatment, colour rule) and correct the sheet's field order to name · budget · colour (D13). Verify no measurement in the entry contradicts what 9.5 measured.

  **Verified**: added a **Reorder Mode** entry under *Components* covering the handle (44×44px, `white/70`→`white/90` on hover-fine), the pushed-back layer (`blur(4px)` + two `--scrim` layers, "roughly a quarter... visible (measured: 36% light, 12% dark)"), its reduced-transparency variant, the lift (`scale(1.02)`, the exact shadow value), and the colour rule. Also added a "Category sheet field order" bullet to *Bottom Sheets & Modals* (name, budget, colour, then Reordenar, then Eliminar categoría). The 36%/12%/blur-4px/≤40%-ceiling figures are copied directly from 9.5's measurements, so nothing contradicts them.
- [x] 11.2 Add the implementation note to `ARCHITECTURE.md` §9 *Modo reordenar*, in the style of the two `add-category-sheet` left: categories only, entered from a menu row rather than a long press, dragged with no press-and-hold, handle outside the card, saved on drop (D13).

  **Verified**: added a `> **Implementado (add-category-reorder-mode, sept 2026):**` blockquote after the "Asas siempre visibles, descartado" paragraph in §9 *Modo reordenar*, matching the style/placement of the two existing `add-category-sheet` notes above it. It states all five points: categories only (the expense-level second tier wasn't built), entered from the "Reordenar" row rather than a long press, no press-and-hold before the drag starts, the handle outside the card in its own lane, and each move saved on drop or keypress rather than on "Listo". `git diff --stat` confirms only these two lines changed.
- [x] 11.3 Add Playwright coverage on `/demo` for: entering and leaving the mode, a keyboard move, a pointer drag and drop, a failed save with its revert and toast, and the reduced-motion variant. Verify the suite passes from a clean run.

  **Verified**: added `tests/reorder-mode.spec.js` (5 tests, 390×844) covering exactly those five cases. The demo has no network layer to intercept for the failure case, so `app/demo/demo-dashboard.tsx` gained a narrow test-only seam — `?e2eFailReorder=1` makes the demo's `reorder()` reject deterministically — gated behind an explicit query param that never fires in normal use. Also gave `playwright.config.js` a `baseURL` and a `webServer` block (`npm run dev`) and added a `"test": "playwright test"` script to `package.json`, so the suite is self-starting. Ran `npx playwright test tests/reorder-mode.spec.js --project=chromium` from a fully clean state (server killed, `test-results/` removed beforehand): **5/5 passed**.
- [x] 11.4 Final verification against the baseline in 1.1: with the cards reordered, every card total, budget level, pie slice, summary row, expenses total (1.700 €) and free margin (974 €) match the baseline exactly, in both themes and both languages.

  **Verified**: a scratchpad Playwright script drove all seven cards into reverse order via the keyboard (the 6.3 gate) on `/demo` at 390×844, across all four combinations of theme (light/dark) and language (es/en), and checked every figure against the 1.1 baseline *by category id*, independent of the new on-screen position:
  - Order was confirmed genuinely reversed (`compras, hogar, salud, transporte, ocio, comida, vivienda`) in all four runs.
  - Every card's total and (where present) budget amount matched the baseline; each budget bar's `aria-valuenow` matched the expected usage percentage within 1%; the "over budget" text appeared for transporte and only transporte.
  - Expenses total (1.700 €) and free margin (974 €) were present in all four runs.
  - The pie chart's legend showed each category's localized name paired with the exact percentage `Intl.NumberFormat(locale, { style: 'percent', maximumFractionDigits: 0 })` computes for its share of the total, in both `es` and `en`.
  - All checks passed with zero failures in all four combinations.
