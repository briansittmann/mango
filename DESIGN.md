...
- **Lift:** The held card scales to `1.03` and carries a two-layer elevation — `0 2px 8px -2px var(--lift-shadow)` for contact, `0 26px 50px -12px var(--lift-shadow)` for height — plus a `0 0 0 1px var(--lift-rim)` ring. `--lift-rim` is transparent in light and a white 14% hairline in dark, because a black shadow on a near-black page separates nothing; dark conveys elevation by catching more light, the same way `--raised-highlight` does. Only `box-shadow` transitions (200ms `--ease-spring` on lift, 260ms on landing) — never `transform`, so the card tracks the finger 1:1 rather than lagging behind it.
- **Lift stacking:** The shadows and the raised `z-index` belong on the card's *wrapper*, not the card. The entrance animation leaves an inline transform on that wrapper, and any transform makes it a stacking context, which traps a z-index set inside it — a card raised only on the inner element slides *under* the cards below it. The wrapper also takes `rounded-card` while held, or its shadows trace a square around a rounded card.
