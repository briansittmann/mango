...
Carrying them SHALL move no existing figure: the cycle's savings total, the accumulated balance, the free margin, the expenses total and every card total, budget bar, pie slice and summary row SHALL be what they were before. The last point of the supplied history SHALL equal the accumulated balance shown in the panel, so the two cannot disagree.
...
- **WHEN** the demo data is built
- **THEN** its savings history carries at least six cycles of accumulated balance
- **AND** its last point equals the accumulated amount shown in the savings panel
