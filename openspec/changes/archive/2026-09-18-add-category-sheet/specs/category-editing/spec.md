...
Every expense card of a category SHALL carry an options control in its header, positioned between the amount and the chevron. The fixed-expenses card SHALL NOT carry one.
...
- **THEN** each of the six category cards has an options control in its header, and the fixed-expenses card has none
...
- the colour picker
- the budget field
- a full-width divider, then "Eliminar categoría" as the last element of the sheet, showing a trash icon and text in the destructive colour

**Sizes:** every field row and the delete row SHALL be at least 48px tall, every control SHALL have a hit area of at least 44 × 44px, no text SHALL compute below 12px, and editable text SHALL compute at 16px or larger.

**Colour:** the brand colour SHALL appear only on the primary action and focus indicators, and the destructive colour only on the delete action and its confirmation. The warning colour SHALL NOT appear.
...
- **AND** from top to bottom it shows the header, the name field, the colour picker, the budget field, a divider and "Eliminar categoría"
...
- **THEN** every field row and the delete row is at least 48px tall
